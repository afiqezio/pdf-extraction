    ...
    <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
          <span class="text-xs text-gray-500"> Page {{ currentPage || 1 }} </span>
          <div class="flex space-x-1">
            <button @click="zoomOut" class="p-1 text-gray-400 hover:text-gray-600" title="Zoom Out">
              <Icon name="heroicons:minus" class="h-4 w-4" />
            </button>
            <span class="text-xs text-gray-500 px-1">{{ Math.round(zoomLevel * 100) }}%</span>
            <button @click="zoomIn" class="p-1 text-gray-400 hover:text-gray-600" title="Zoom In">
